...
namespace Assistant.MapUO
{
    public class UOMapControl : PictureBox
    {
        ...
        private static readonly Font m_BoldFont = new Font("Courier New", 8, FontStyle.Bold);
        private static Font m_SmallFont = new Font("Arial", 6);
        private static readonly Font m_RegFont = new Font("Arial", 8);
        private DateTime LastRefresh;

        private bool m_Active;
        private Bitmap m_Background;
        private Mobile m_Focus;
        private readonly Region[] m_Regions;
        ...
        public UOMapControl()
        {
            ...
            m_Regions = MapUO.Region.Load("guardlines.def");
            MapButtons = UOMapRuneButton.Load("test.xml");
        }
        ...
        public ArrayList MapButtons { get; set; }
        ...
        public void FullUpdate()
        {
            ...
            int xtrans = Width / 2;
            int ytrans = Height / 2;
            Point3D focus = FocusMobile.Position;
            ...
            Ultima.Map map = Map.GetMap(FocusMobile.Map);
            ...
            if (Width > Height)
            {
                regions = RegionList(focus.X, focus.Y, Width);
                mButtons = ButtonList(focus.X, focus.Y, Width);
            }
            else
            {
                regions = RegionList(focus.X, focus.Y, Height);
                mButtons = ButtonList(focus.X, focus.Y, Height);
            }
            ...
            try
            {
                if (Active)
                {
                    ...
                    foreach (Serial s in PacketHandlers.Party)
                    {
                        ...
                        if (mob == FocusMobile && mob == World.Player)
                            continue;
        ...
        private UOMapRuneButton ButtonCheck(Rectangle rec)
        {
            ...
            foreach (UOMapRuneButton mbutton in MapButtons)
            {
        ...
        private ArrayList RegionList(int x, int y, int maxDist)
        {
            ...
            {
                Region rg1 = m_Regions[i];
                if (Utility.Distance(rg1.X, rg1.Y, x, y) <= maxDist * 2) aList.Add(rg1);
            }
        ...
        private ArrayList ButtonList(int x, int y, int maxDist)
        {
            ...
            int count = MapButtons.Count;
            ArrayList aList = new ArrayList();

            for (int i = 0; i < count; i++)
            {
                UOMapRuneButton btn = (UOMapRuneButton) MapButtons[i];
                if (Utility.Distance(btn.X, btn.Y, x, y) <= maxDist * 2) aList.Add(btn);
            }
