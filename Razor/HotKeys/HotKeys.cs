...
namespace Assistant
{
    ...
    {
        private readonly HotKeyCallback m_Callback;
        private readonly HotKeyCallbackState m_CallbackState;
        private int m_Key;
        ...
        public object pState => m_State;

        public string DispName => LocName != 0 ? Language.GetString(LocName) : StrName;
        public int LocName { get; }
        public string StrName { get; }

        public bool SendToUO { get; set; }
        ...
        {
            if (Key != 0)
                return string.Format("{0} ({1})", DispName, KeyString());

            return string.Format("{0} ({1})", DispName, Language.GetString(LocString.NotAssigned));
        }
    }
    ...
    public class HotKey
    {
        ...
        static HotKey()
        {
            ...
            RootNode.Tag = (int) LocString.HotKeys;
        ...
        public static List<KeyData> List { get; } = new List<KeyData>();
        public static TreeNode RootNode { get; } = new TreeNode("Hot Keys");
        ...
        {
            m_HK_En = Add(HKCategory.None, LocString.ToggleHKEnable, OnToggleEnable);
        }
        ...
        private static void UpdateNode(TreeNode node)
        {
            if (node != null)
            {
                ...
                else if (node.Tag is int)
                    node.Text = Language.GetString((int) node.Tag);

                if (node != RootNode && node.NextNode != null)
        ...
        {
            return FindParent(RootNode, cat, sub);
        }
        ...
        {
            return MakeNode(RootNode, text, tag);
        }
