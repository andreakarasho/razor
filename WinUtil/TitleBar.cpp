...
static inline int Round(float n) {
	int i = (int)n;
	return i + (n - i >= 0.5 ? 1 : 0);
}

static unsigned short *GetHue(int index) {
	if (Hues == NULL) {
		char str[512];
...
static unsigned short ApplyHueToPixel(unsigned short *hue, unsigned short pix) {
	if (hue)
...
static UOItem *ReadUOItem(int item, int bh) {
...
static UOItem *FindItem(int item) {
...
static inline COLORREF Brightness(int shift, COLORREF c) {
	return RGB(min(255, GetRValue(c) + shift), min(255, GetGValue(c) + shift), min(255, GetBValue(c) + shift));
}

static int DrawUOItem(HDC hDC, RECT rect, int item, int hueIdx) {
	item |= 0x4000;
...
static void FreeItem(UOItem *node) {
	if (node != NULL) {
		FreeItem(node->pNext);
...
{
	strncpy(DataPath, datapath, sizeof(DataPath));
...
static inline int GetHex2( LPCSTR hex )
{
	int num = 0;
...
static inline int GetHex4( LPCSTR hex )
{
	return (GetHex2( hex ) << 8) | GetHex2( &hex[2] );
}

static void DoStat( HDC hDC, int v, int t, int l, int h, int w )
{
	if ( w <= 0 )
		return;
	RECT fill;
...
static int DrawStatBar( HDC hDC, RECT rect, int width, int status, int hp, int mn, int st )
{
	HGDIOBJ hOld = NULL;
	POINT pt[2];
...
static void DrawColorTitleBar( HTHEME hTheme, HWND hWnd, HDC hOutDC, bool active, bool maximized, LPCSTR str, int len, RECT orig )
{
...
DLLFUNCTION void DrawTitleBar( HWND hWnd, const char *str )
{
	...
	if ( hThemes )
	{
		...
		DrawColorTitleBar( hTheme, hWnd, hDC, true, place.showCmd == SW_MAXIMIZE, str, len, rect );
		zCloseThemeData( hTheme );
	}
	else
	{
		rect.left += GetSystemMetrics(SM_CXFRAME);
		DrawColorTitleBar( NULL, hWnd, hDC, true, place.showCmd == SW_MAXIMIZE, str, len, rect );
	}
